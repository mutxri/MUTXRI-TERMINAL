#!/usr/bin/env python3
"""fetch_egx_max.py - fetch since-IPO MONTHLY history (.max.json) for EGX
securities using TICKER.CA. Slow pace (3s) + retries. Same proven pattern as
fetch_egx_history.py. Skips files with >=2 bars."""
...
def yahoo_chart(sym, rng="max", ivl="1mo"):
...
def main():
    ...
    for i, s in enumerate(egx):
        ...
        if not sym or not tkr:
            continue
        out = os.path.join(HIST, sym.replace("/", "_") + ".max.json")
