...
for p in STATIC_PANELS:
    # static_data holds the snapshot variants; features/panels holds the
    # canonical (version-controlled) source. Prefer the snapshot, fall back to
    # the source so a panel that needs no snapshot variant still ships.
    ...
    if os.path.exists(src):
        ...
        print(f"  {origin} panel: {p}")
    else:
        print(f"  MISSING panel: {p}")
